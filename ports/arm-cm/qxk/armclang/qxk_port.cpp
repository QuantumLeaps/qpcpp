                    ...
                    void *stkSto, uint_fast16_t stkSize)
{
    ...
    // fabricate the ARM Cortex-M exception stack frame...
    ...
    *(--sp) = 0xAAAAAAAAU;   // stack "aligner"
    ...
    sp_limit = reinterpret_cast<uint32_t *>(
        (((reinterpret_cast<uint32_t>(stkSto) - 1U) >> 3) + 1U) << 3);
    for (; sp >= sp_limit; --sp) {
        *sp = 0xDEADBEEFU;
    }
}
...
//Q_ASSERT_COMPILE(QMACTIVE_OSOBJ == offsetof(QP::QActive, m_osObject));
//Q_ASSERT_COMPILE(QMACTIVE_PRIO == offsetof(QP::QActive, m_prio));
