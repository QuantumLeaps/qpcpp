...
if "%1"=="rel" (
    echo rel selected
    set BINDIR=rel
    set CCFLAGS=-I%IAR_ARM%\arm\inc -D N_DEBUG --endian little --cpu %ARM_CORE% --eec++ -e --fpu None --dlib_config %IAR_ARM%\ARM\INC\c\DLib_Config_Normal.h --diag_suppress=Pa050 --interwork -Ohz
    set ASMFLAGS=-s+ -w+ -r --cpu %ARM_CORE% --fpu None -I%IAR_ARM%\arm\inc
)
if "%1"=="spy" (
    echo spy selected
...
%CC% --cpu_mode arm   %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qvanilla.cpp
...
@echo on
%CC% --cpu_mode arm   %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qs.cpp
...
%CC% --cpu_mode thumb %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qs_f32.cpp
%CC% --cpu_mode thumb %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qs_f64.cpp
%CC% --cpu_mode thumb %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qs_mem.cpp
