...
struct QF_cyg_mbox {                         // event queue structure for eCos
    cyg_handle_t handle;                             // handle to eCos mailbox
    cyg_mbox     mbox;                               // eCos mailbox structure
};

struct QF_cyg_thread {                            // thread structure for eCos
...
//
struct QF_cyg_mempool_fix {                             // event pool for eCos
...
#define QF_EPOOL_GET_(p_, e_) do { \
    (e_) = static_cast<QEvt *>(cyg_mempool_fix_try_alloc((p_).handle)); \
    Q_ENSURE((e_) != static_cast<QEvt *>(0)); \
} while (0)

#define QF_EPOOL_PUT_(p_, e_) do { \
    Q_REQUIRE((e_) != static_cast<QEvt *>(0)); \
