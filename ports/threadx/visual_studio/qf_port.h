...
#define QF_TX_PRIO_OFFSET   8
                                 ...
                                 // QF interrupt locking/unlocking for ThreadX
...
struct ThreadXBlockPool {                  // TreadX block pool and block-size
    TX_BLOCK_POOL m_pool;                                // ThreadX block pool
    QP_ QEvtSize m_block_size;                   // the block size of the pool
};
...
#define QF_EPOOL_INIT_(p_, poolSto_, poolSize_, evtSize_) do { \
    (p_).m_block_size = static_cast<QEvtSize>(evtSize_); \
    Q_ALLEGE(tx_block_pool_create(&(p_).m_pool, "P", \
        (evtSize_), (poolSto_), (poolSize_)) == TX_SUCCESS); \
} while (false)

#define QF_EPOOL_EVENT_SIZE_(p_) \
    static_cast<uint32_t>(((p_).m_block_size))

#define QF_EPOOL_GET_(p_, e_)   do { \
    if (tx_block_allocate(&(p_).m_pool, \
            reinterpret_cast<void **>(&(e_)), TX_NO_WAIT) != TX_SUCCESS) \
    { \
