/// \file
/// \brief QP::QF_pool_[] definition and QP::QF::poolInit() implementation.
/// \ingroup qf
/// \cond
///***************************************************************************
/// Product: QF/C++
...
void QF::poolInit(void * const poolSto,
                  uint_fast16_t const poolSize, uint_fast16_t const evtSize)
{
    /// \pre cannot exceed the number of available memory pools
    Q_REQUIRE_ID(100, QF_maxPool_
                      < static_cast<uint_fast8_t>(Q_DIM(QF_pool_)));
    /// \pre please initialize event pools in ascending order of evtSize
    Q_REQUIRE_ID(101, (QF_maxPool_ == uf8_0)
                      || (QF_EPOOL_EVENT_SIZE_(QF_pool_[QF_maxPool_ - uf8_1])
                          < evtSize));
