    ...
    std::uint_fast8_t const qsId)
{
    ...
    if (r >= Q_RET_TRAN) { // any kind of tran. taken?
        ...
        Q_ASSERT_INCRIT(330, ts != nullptr);
        QF_CRIT_EXIT();
    #endif // Q_SPY
    ...
    // event bubbled to the 'top' state?
        ...
        QS_CRIT_ENTRY();
        // current state can't be nullptr
        Q_ASSERT_INCRIT(390, s != nullptr);
    ...
    std::uint_fast8_t const qsId)
{
    ...
    for (; (s != ts) && (lbound > 0); --lbound) {
        ...
        if (s == nullptr) { // reached the top of a submachine?
            ...
            QF_CRIT_ENTRY();
            Q_ASSERT_INCRIT(880, s != nullptr); // must be valid
            QF_CRIT_EXIT();
        }
    }
    QF_CRIT_ENTRY();
