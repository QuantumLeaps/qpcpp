...
void QActive::register_() noexcept {
    ...
    Q_REQUIRE_INCRIT(110, registry_[m_prio] == nullptr);
    Q_REQUIRE_INCRIT(120, m_prio <= m_pthre);

    std::uint8_t prev_thre = m_pthre;
    std::uint8_t next_thre = m_pthre;

    for (std::uint_fast8_t p = static_cast<std::uint_fast8_t>(m_prio) - 1U;
         p > 0U;
         --p)
    {
...
void QActive::unregister_() noexcept {
    ...
    Q_REQUIRE_INCRIT(210, registry_[p] == this);
