...
glb_filter(GRP_ALL)
continue_test()
expect("===RTC===> St-Init  Obj=AO_Gizmo,State=QP::QHsm::top->Gizmo::init")
expect("@timestamp AO-Post  Sdr=AO_Gizmo,Obj=AO_Gizmo,Evt<Sig=START_SIG,*")
expect("===RTC===> St-Entry Obj=AO_Gizmo,State=Gizmo::init")
expect("@timestamp Init===> Obj=AO_Gizmo,State=Gizmo::init")
expect_run()
expect("@timestamp AO-GetL  Obj=AO_Gizmo,Evt<Sig=START_SIG,*")
expect("@timestamp Disp===> Obj=AO_Gizmo,Sig=START_SIG,State=Gizmo::init")
expect("@timestamp USER+005 START")
expect("@timestamp ===>Tran Obj=AO_Gizmo,Sig=START_SIG,State=Gizmo::init->Gizmo::on_mains")
...
expect("@timestamp Disp===> Obj=AO_Gizmo,Sig=START_SIG,State=Gizmo::init")
expect("@timestamp USER+005 START")
expect("@timestamp ===>Tran Obj=AO_Gizmo,Sig=START_SIG,State=Gizmo::init->Gizmo::on_battery")
