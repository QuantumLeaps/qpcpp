//$file${.::calc.cpp} vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
//
// Model: calc.qm
// File:  ${.::calc.cpp}
//
...
//
//$endhead${.::calc.cpp} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#include "qpcpp.hpp"  // QP/C++ interface
...
Q_STATE_DEF(Calc, ready) {
    ...
    switch (e->sig) {
        ...
        case OPER_SIG: {
            m_operand1 = BSP_get_value();
            m_operator = Q_EVT_CAST(CalcEvt)->key_code;
...
Q_STATE_DEF(Calc, operand2) {
    ...
    switch (e->sig) {
        ...
        case EQUALS_SIG: {
            ...
            if (BSP_eval(m_operand1, m_operator, BSP_get_value())) {
                status_ = tran(&result);
            }
