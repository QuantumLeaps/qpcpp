...
/*
    ...
    QF_CRIT_STAT
    QF_CRIT_ENTER();
    GPIO->P[LED_PORT].DOUBT |=  (1U << LED1_PIN); // turn the LED on
    GPIO->P[LED_PORT].DOUBT &= ~(1U << LED1_PIN); // turn the LED off
    QF_CRIT_EXIT();
*/
...
void BSP::randomSeed(uint32_t seed) {
    ...
    l_rndMutex = OSMutexCreate(QF_MAX_ACTIVE - (N_PHILO + 3U), &err);
